...
public:
    ...
    {
        // Clear all content from the list
        clear();
        // Create Pointers
...
public:
    ...
    // Copy Constructor
    DoublyLinkedList(const DoublyLinkedList<char>& other) { (*this) = other; }
    // constuctor w/ data
    DoublyLinkedList(const char& m_data)
    {
        Node<char>* node = new Node<char>(m_data);
        m_head = node;
        m_tail = node;
        //make the list circular
        m_head->previous = m_tail;
    ...
    {
        Node<char>*tmp = new Node<char>(data);
        // list empty?
        if (! m_head)
        {
            m_head = tmp;
    ...
    void insert(int index, char data)
    {
        ...
        // create new Node
        Node<char>*p = new Node<char>(data);
        // create a Node ptr and point at head
        Node<char>*temp;
        temp = m_head;
        // move through list till you reach index
    ...
    //Find the first occurence of "search char" in the linked list and return its position. Return -1 if the character is not found.
    int findFirstOf(const char& search) const
    {
        int index = 0;
        Node<char>*temp;
        temp = m_head;
    ...
    // Overload the = operator
    DoublyLinkedList<char>& operator=(const DoublyLinkedList<char>& other)
    {
        clear();
        if (other.m_node_count > 0)
        {
            Node<char>* o_current, *t_current;
            m_head = new Node<char>(other.m_head->data);
            t_current = m_head;
            o_current = other.m_head->next;
            for (int i = 1; i < other.m_node_count; i++)
            {
                t_current->next = new Node<char>(o_current->data);
                t_current->next->previous = t_current;
                o_current = o_current->next;
                t_current = t_current->next;
            }
    ...
    // Return First Node
    Node<char>* firstNode() { return m_head; }

    // Return last node
    Node<char>* lastNode() { return m_tail; }
